...
public class HuaweiScheduler4Task {
    ...
    public void execute() throws Exception {
        ...
        for (TaskHW t : taskhwList) {
                logger.info(new Date()+"[下发任务调度]:任务-[" + t.getTaskId() + "]开始下发!");
                try {
                    //根据任务id获取ip
...
//                    throw new RuntimeException("[下发任务调度]: 下发任务失败，远程存在同名任务请先删除或重新下订单!");
                ...
                taskhwService.update(t);
                logger.info(new Date()+"[在华为设备创建监控任务]:任务-[" + t.getTaskId() + "]完成创建!");
        }
                ...
                try {
                    ...
                    if(alarmList.get(0).getEnd_time()!=null){
                       ...
                       //根据任务id获取ip
                        OrderIP ip = taskhwService.getIpByTaskId(t.getOrder_ip_Id());
                        //删除引流任务
                        //HuaweiWorker.lssuedDeleteDivertTask(ip.getIp());
                        //更新任务引流状态为2,已删除引流
                        t.setDrainage(2);
                        taskhwService.update(t);
                    }
...
//                }
                logger.info("[删除监控任务调度]:任务-[" + t.getTaskId() + "]完成下发!");
        }
