...
/**
 * 创 建 人 ： tangxr 创建日期： 2016-5-5 描 述： 订单告警管理控制层 版 本： 1.0
 */
@Controller
...
//	}
	...
	public String warningInit(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (id == "null" || "".equals(id)) {
			...
			if (false) {
				...
				/*
				 * //当任务状态为3，完成状态 int status = Integer.parseInt(Constants.TASK_FINISH);
				 * request.setAttribute("status", status); //获取进度 Map<String, Object> pMap = new
				 ...
				 * request.setAttribute("currentUrl", "无"); } }else{ if(status==3){
				 * request.setAttribute("progress", 100); request.setAttribute("currentUrl",
				 * "无"); }else{ request.setAttribute("progress", 0);
				...
				/** 基本信息 dyy */
				/*
				 * Task task = new Task(); // if(Integer.parseInt(type)==1){ // //长期查找最近的任务 //
	...
	public String historyInit(HttpServletRequest request) {
		...
		/**
		 * 
		 * String orderId = request.getParameter("orderId"); //获取订单信息 List orderList =
		 * orderService.findByOrderId(orderId); //获取对应告警信息 Map<String, Object> paramMap
		 * = new HashMap<String, Object>(); paramMap.put("orderId", orderId);
		 * List<Alarm> alarmList = alarmService.getAlarmByOrderId(paramMap); //获取检测次数
