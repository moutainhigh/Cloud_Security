...
/**
 * 创 建 人 ： tangxr 创建日期： 2016-05-25 描 述： waf接口本地Worker 版 本： 1.0
 */
public class WafAPIWorker {
	...
	static {
		try {
			...
			p.load(WafAPIWorker.class.getClassLoader().getResourceAsStream("InternalAPI.properties"));
	...
	public static String getSites(String resourceId, String deviceId) {
		...
		// 获取响应结果
		ClientResponse response = service.type(MediaType.APPLICATION_JSON_TYPE).post(ClientResponse.class,
				json.toString());
	 ...
	 * 
	 * @param resourceId
	 *            设备资源编号， deviceId 设备编号， ip 设备ip， mask 设备编号
	 * @time 2016-5-25
	 */
	 ...
	 * 
	 * @param resourceId
	 *            设备资源编号， deviceId 设备编号， name 站点名称， ip 设备ip， port 站点端口， cert 证书名称，
	 *            type 协议类型
	 * @time 2016-5-25
	 */
	public static String createSite(String resourceId, String deviceId, String name, String ip, String port,
	 ...
	 * 
	 * @param resourceId
	 *            设备资源编号， deviceId 设备编号， siteId 站点id， name 站点名称， ip 设备ip， port 站点端口，
	 *            cert 证书名称， type 协议类型
	 * @time 2016-5-25
	 */
	public static String alterSite(String resourceId, String deviceId, String siteId, String name, String ip,
			String port, String cert, String type) {
		// 组织发送内容JSON
		JSONObject json = new JSONObject();
		json.put("resourceId", "10001");
		json.put("deviceId", "30001");
	 ...
	 * 
	 * @param resourceId
	 *            设备资源编号， deviceId 设备编号， parent name 站点名称， ip 设备ip， port 站点端口， cert
	 *            证书名称， type 协议类型
	 * @time 2016-5-25
	 */
	public static String createVirtualSiteInResource(String resourceId, String name, String wafIp, String wafPort,
			String cert, String type, String domain, String include, String exclude, JSONArray server) {
		// 组织发送内容JSON
		JSONObject json = new JSONObject();
	...
	/**
	 * 功能描述：删除waf 参数描述： targetKey 订单编号
	 */
	...
	/**
	 * 功能描述：post方法请求 参数描述:String sessionId 回话ID, String taskId任务ID
	 * 
	 * @time 2015-12-31
	 */
	private static String postMethod(String url, String xml, String sessionId) {
		...
		WebResource service = client.resource(url);
		// 获取响应结果
		String response = service.cookie(new NewCookie("sessionid", sessionId)).type(MediaType.APPLICATION_XML)
				.accept(MediaType.TEXT_XML).post(String.class, xml);
		return response;
	}
	...
	/**
	 * 功能描述：安全通信配置设置 参数描述:String url 路径,ClientConfig config 配置对象
	 * 
	 * @time 2015-12-31
	 */
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		 ...
		 * 
		 * String createSite = createSite("10001", "30001", "new", "187.6.0.2", "80",
		 * "", "1"); String createSiteRe = controller.analysisGetcreateSite(createSite);
		 * System.out.println(createSiteRe);
		 */

		// System.out.println(eth);
		// String ethStr =
		// "{\"ip_address\":[{\"ip\":\"187.6.0.1\",\"mask\":\"255.255.255.0\",\"multi_status\":200,\"multi_result\":\"created
		// successfully\"}]}";
		/*
		 * JSONObject json = new JSONObject(); json.put("ip", "192.168.1.11");
		 * json.put("port", "62222"); JSONArray ser = JSONArray.fromObject(json); String
		 * virtualSite = createVirtualSite("10001", "30001", "1464833085", "testfire",
		 * "http://www.testfire.net/", "*", "", ser); String virtualSiteRe =
		 * controller.analysisGetCreateVirtualSite(virtualSite);
		...
		/*
		 * String wafLogByIp = getWaflogWebsecByIp(dstIpList); List list =
		 * controller.analysisGetWaflogWebsecByIp(wafLogByIp);
		 * if(list!=null&&list.size()>0){ for (int i = 0; i < list.size(); i++) { Map
		 * map = (Map)list.get(i); int logId = (Integer)map.get("logId"); int resourceId
		 * = (Integer)map.get("resourceId"); String resourceUri =
		 * map.get("resourceUri").toString(); String resourceIp =
		 * map.get("resourceIp").toString(); long siteId = (Long)map.get("siteId"); int
		 * protectId = (Integer)map.get("protectId"); String dstIp =
		 * map.get("dstIp").toString(); String dstPort = map.get("dstPort").toString();
		 * String srcIp = map.get("srcIp").toString(); String srcPort =
		 * map.get("srcPort").toString(); String method = map.get("method").toString();
		 * String domain = map.get("domain").toString(); String uri =
		 * map.get("uri").toString(); String alertlevel =
		 * map.get("alertlevel").toString(); String eventType =
		 * map.get("eventType").toString(); String statTime =
		 * map.get("statTime").toString(); String action = map.get("action").toString();
		 * String block = map.get("block").toString(); String blockInfo =
		 * map.get("blockInfo").toString(); String alertinfo =
		 * map.get("alertinfo").toString(); String proxyInfo =
		 * map.get("proxyInfo").toString(); String characters =
		 * map.get("characters").toString(); int countNum =
		 * (Integer)map.get("countNum"); String protocolType =
		 * map.get("protocolType").toString(); String wci = map.get("wci").toString();
		...
		/*
		 * String wafLogById = getWaflogWebsecById("2104"); Map map =
		 * controller.analysisGetWaflogWebsecById(wafLogById);
		 * 
		 * int logId = (Integer)map.get("logId"); int resourceId =
		 * (Integer)map.get("resourceId"); String resourceUri =
		 * map.get("resourceUri").toString(); String resourceIp =
		 * map.get("resourceIp").toString(); long siteId = (Long)map.get("siteId"); int
		 * protectId = (Integer)map.get("protectId"); String dstIp =
		...
		/*
		 * String wafLogInTime = getWaflogWebsecInTime(dstIpList,"1"); List list3 =
		 * controller.analysisGetWaflogWebsecInTime(wafLogInTime);
		 * if(list3!=null&&list3.size()>0){ for (int i = 0; i < list3.size(); i++) { Map
		 * map = (Map)list3.get(i); int logId = (Integer)map.get("logId"); int
		 * resourceId = (Integer)map.get("resourceId"); String resourceUri =
		 * map.get("resourceUri").toString(); String resourceIp =
		 * map.get("resourceIp").toString(); long siteId = (Long)map.get("siteId"); int
		 * protectId = (Integer)map.get("protectId"); String dstIp =
		 * map.get("dstIp").toString(); String dstPort = map.get("dstPort").toString();
		...
		try {
			/*
			 * JSONObject obj = JSONObject.fromObject(ethStr); String ip_address =
			 * obj.getString("ip_address"); if("success".equals(status)){
			 * System.out.println(status); }
			 */
			/*
			 * JSONObject siteObject = JSONObject.fromObject(ethStr); JSONObject
			 * ip_addressObject = siteObject.getJSONObject("ip_address"); JSONObject
			 * ipObject = ip_addressObject.getJSONObject("ip"); String mask =
			 * ipObject.getString("mask"); System.out.println(mask);
			 */
			// virtualSite = createVirtualSite("10001", "30001", "1464833085", "testfire",
			...
			// jo.put("port", "80");
			// ser.add(jo);
			// wafcreate = WafAPIWorker.createVirtualSiteInResource("10001", "test0615",
			// "219.141.189.183", "80", "nsfocus.cer", "0", "www.anquanbang", "*", "", ser);
