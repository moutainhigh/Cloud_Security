...
public class MaliciousUrlValidDataController {
	...
	public void worldMapURL(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		String url=bundle.getString("Country_Valid_Url");//按国家获得有效的恶意URL个数
		String allCountUrl=bundle.getString("Malurl_All_COUNT");//获得全球恶意URL个数
	...
	public void getTopData(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		JSONArray countList=(JSONArray) jb.get("webphishList");
		JSONArray jsonArray=new JSONArray();
		Iterator<Object>it=countList.iterator();
		while(it.hasNext()){
			JSONObject ob=(JSONObject) it.next();
			JSONObject jsonObject=new JSONObject();
			jsonObject.put("id", ob.getString("id"));
			jsonObject.put("url", ob.getString("url"));
			jsonObject.put("ip", ob.getString("ip"));
			jsonObject.put("city", ob.getString("city"));
			String index=ob.getString("countryCode");
			if(index==null||index.length()<2){
				jsonObject.put("country"," " );
			}else{
				jsonObject.put("country", map.get(index));
			}
