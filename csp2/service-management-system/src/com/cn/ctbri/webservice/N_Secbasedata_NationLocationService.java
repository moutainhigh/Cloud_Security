...
@Component
@Path("openapi/secbasedata/nationlocation")
public class N_Secbasedata_NationLocationService {
	...
	public String getNationLocationCNCount(@PathParam("token") String token) {
		JSONObject json = new JSONObject();
		try {
			//通过token查询user
			System.out.println("aaaaaaaaa  +  getNationLocationCNCount()");
					...
					if(!serviceExpired(user,oAPIList)){
						if(!usedUp(user, userableList)){
							...
							for (int i = 0; i < oAPIList.size(); i++) {
								countAPI(token, oAPIList.get(i).getId(), null, 7, 2);
							}
	...
	@Produces(MediaType.APPLICATION_JSON) 
	public String getNationLocationCNDataBlock(@PathParam("token") String token, String dataJson) {
		JSONObject json = new JSONObject();
	...
	@Produces(MediaType.APPLICATION_JSON) 
	public String getNationLocationTotalCount(@PathParam("token") String token) {
		JSONObject json = new JSONObject();
		try {
			//通过token查询user
			System.out.println("aaaaaaaaa  +  getNationLocationCNCount()");
	...
	//获取全球地理信息数据块    南向 有问题   在调试 
	@POST
    @Path("/getdatablock/{token}")
	@Produces(MediaType.APPLICATION_JSON) 
	public String getNationLocationDataBlock(@PathParam("token") String token, String dataJson) {
		JSONObject json = new JSONObject();
		JSONObject jsonObj = new JSONObject().fromObject(dataJson);
		String begIndex = jsonObj.getString("begIndex");
		String endIndex = jsonObj.getString("endIndex");
	...
	/**
	 * 用来获取国内地理信息数据块 和 全球地理信息数据块 以及 获取国内地理信息数据总数 加上 获取全球地理信息数据总数 专用包装类
	 * @param apiReturn
	 * @param json
	 */
