...
//					}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String VulnScan_Get_orderReport(@PathParam("orderId") String orderId,@Context HttpServletRequest request,@Context HttpServletResponse response) {
		JSONObject json = new JSONObject();
		try {
			//查找订单
			Order order = orderService.findOrderByOrderId(orderId);
...
//        		file = downLoadResourceHandler.getResourceStream(resourceID, userID);
...
//				return HttpDownloader.download( file, length, request);
			}else{
				json.put("code", 421);
				json.put("message", "订单不存在");
			}
