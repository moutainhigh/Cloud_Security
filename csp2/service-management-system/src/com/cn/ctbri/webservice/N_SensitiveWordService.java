 ...
 * 版        本：  1.0
 */
//2017-05-10 网页敏感内容监测服务

@Component
@Path("openapi/sensitiveword")
public class N_SensitiveWordService {
	@Autowired
	...
	public String VulnScan_opt_orderTask(@PathParam("token") String token, @PathParam("orderId") String orderId, String dataJson) {
		...
		try {
			...
			if(token!=null && token!="" && user!=null){
				...
				if(order!=null && order.getServiceId()==4 && (apikey||userId)){
					JSONObject jsonObj = new JSONObject().fromObject(dataJson);
		...
		//	        	}else if(o.getStatus()==5){
			        	...
			        	this.countAPI(token, orderId, null, 4, 2);
...
//								json.put("websoc", order.getWebsoc());
				...
				this.countAPI(token, orderId, null, 4, 3);
	...
	//						return json.toString();
				...
				this.countAPI(token, orderId, taskId, 4, 4);
			}else{
				json.put("code", 422);
				json.put("message", "token无效");
			}
...
//		JSONObject json = new JSONObject();
		try {
			...
			if(order!=null && order.getServiceId()==4 && (apikey||userId)){
				this.countAPI(token, orderId, null, 4, 5);
