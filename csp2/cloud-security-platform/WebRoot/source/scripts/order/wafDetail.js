		     ...
		     success: function(data) {
    			   		 if(!data.flag){
    			   			...
    			   			$.ajax({ type: "POST",
	    			   		     ...
	    			   		     dataType: "json", 
	    			   		     success: function(data) {
	    			   		    	 window.location.href="buyNowWafUI.html?type="+2+"&beginDate="+beginDate+"&endDate="+endDate+"&scanType="+scanType+"&serviceId="+serviceId+"&domainName="+domainName+"&domainId="+domainId+"&price="+price;
	    			   		    	 }, 
	    			   		     error: function(data){ 
	    			   		    	 if (data.responseText.indexOf("<!DOCTYPE html>") >= 0) { 
	    			   		    		 window.location.href = "loginUI.html"; } 
