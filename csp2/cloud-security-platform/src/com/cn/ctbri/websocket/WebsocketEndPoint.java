...
public class WebsocketEndPoint extends TextWebSocketHandler {  
	...
	public void afterConnectionEstablished(WebSocketSession session)
		...
		System.out.println("Kaishi1234-555----------------------------------");
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {  
        ...
        WafAPIWorker worker=new WafAPIWorker();
    	String text=worker.getAllWafLogWebsecInTime("10","minute");
    	...
    	for(int i=0;i<size;i++){
			...
			String srcName=null;
			String desName=null;
			if("219.142.69.76".equals(srcIP)||"219.142.69.76".equals(desIP)){
				System.out.println("--------");
			}
			...
			if(null==srcIPPosition){//数据库中不存在
				...
				srcName=MapUtil.getCountryByPosition(srcLongitude, srcLatitude);
			...
			if(null==desIPPosition){//数据库中不存在数据
				...
				desName=MapUtil.getCountryByPosition(desLongitude, desLatitude);
			...
			if(StringUtils.isNotEmpty(srcName)&&StringUtils.isNotEmpty(desName)){
				//组装数据
