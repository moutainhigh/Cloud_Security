...
import com.cn.ctbri.southapi.adapter.waf.nsfocus.syslog.WAFSyslogConfig;
	...
	private boolean loadWAFConfigSyslogServer(List<?> list) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if ( !WAFSyslogConfig.DEFAULT_PACKET_DISPOSEMODE_RELAY.equalsIgnoreCase(packetDisposeMode) )  //NOT Relay : Set default to store
					packetDisposeMode =  WAFSyslogConfig.DEFAULT_PACKET_DISPOSEMODE_STORE;
				WAFSyslogConfig.setPacketDisposeMode(packetDisposeMode);
				
				if ( "Relay".equalsIgnoreCase(packetDisposeMode)) {
					List<?> listSyslogServerStore = elementSyslogServer.selectNodes("Relay");
					loadWAFConfigSyslogServerRelay(listSyslogServerStore);
				} else  {
					List<?> listSyslogServerRelay = elementSyslogServer.selectNodes("Store");
					loadWAFConfigSyslogServerStore(listSyslogServerRelay);				
				}
