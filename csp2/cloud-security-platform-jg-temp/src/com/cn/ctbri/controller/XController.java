...
public class XController {
	...
	private static String getScanServiceReportUrl=bundle.getString("getscanservicereport_Url");
	private static String creatScanServiceReportUrl=bundle.getString("createscantask_Url");
	...
	public void detectionUrl(HttpServletRequest request,HttpServletResponse response){
	  ...
	  public boolean PostDetection(String urlInfo) {
			...
			@ResponseBody
			public void portScanMethod2(HttpServletRequest request, HttpServletResponse response){
				/*
				String target="tcp";
				String port="34,45,78,89-102";
				String scan="tcp";
				m172490z30.imwork.net
				anquanbang.net
				baidu.com
				google.com
				21-22,80,443
				String agreement="tcp";*/
				//JerseyJsonUtil jerseyJsonUtil=new JerseyJsonUtil();
				//String status=JSONObject.fromObject(jerseyJsonUtil.getMethod(SOUTH_SERVER_WEB_ROOT+getScanServiceReportUrl)).get("status").toString();
				...
				for(String str:strArr){
					...
					if(str.contains("-")){
						...
						int end =Integer.valueOf(str.substring(number+1, str.length()).trim());
						if(begin>end){
							int middle=end;
							end=begin;
							begin=middle;
			...
			@ResponseBody
			public void portScanMethod(HttpServletRequest request, HttpServletResponse response){
				/*
				String target="tcp";
				String port="34,45,78,89-102";
				String scan="tcp";
				...
				JerseyJsonUtil jerseyJsonUtil=new JerseyJsonUtil();
				String count=JSONObject.fromObject(jerseyJsonUtil.
						postMethod(getScanServiceReportUrl,jsonObject2.toString())).toString();
				JSONObject result=new JSONObject();
				String target=request.getParameter("target");
				String port=request.getParameter("port");
				String scan=request.getParameter("scan");
				String agreement=request.getParameter("port");
				if(count==null||count.length()==0||(!count.startsWith("{"))||
						!JSONObject.fromObject(count).getString("status").equals("success")){
				...
				if(jsonArray!=null&&jsonArray.size()>0){
					...
					String dataTime=timeFormat.format(Long.valueOf(report.getString("addTime")));
